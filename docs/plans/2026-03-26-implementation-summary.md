# HackerNews 日本語まとめ & AI要約 MVP 実装サマリー
...
本サービスの月間 Actions 消費量は約 90〜150分で、private repo の無料枠 2,000分/月の 10% 以下。コードを守りたい場合も、private repo 単体で十分に目的を達成できる。
...
3. **GitHub Actions を手動実行して動作確認**
   - Actions タブ → "HackerNews 日本語まとめ & AI要約 毎日更新" → "Run workflow"
...
| Y Combinator 商標クレーム | フォールバック名: `ハクニチ`、`テックまとめJP`、`HN今日のまとめ` |
