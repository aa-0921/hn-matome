...
冒頭の **「■ 未対応はあるか？」** と **表 A/B/C** と同一内容を、やや詳しくまとめたもの。

- **計画項目の未対応**: **0 件**（1-1〜8-2 はすべて「済」）。
- **任意で未着手**: **About へのスロット長文**（4-4）、**コントラスト検証の数値記録**（3-3）。必須ではない。
- **非採用・運用制約**（項目の未実装ではない）:
...
| 日付 | 内容 |
|------|------|
| 2026-03-29（初回） | `style.css` 全面（変数・ダーク・focus・sticky・モバイルナビ・date-nav・戻るボタン等）、`base.html`（スキップ・検索/Pagefind・相対時刻日本語・details 状態・一括開閉）、`index.html` / `archive.html`（ARIA・ツールバー・アーカイブ導線）、`archive_index.html`・`generator.generate_archive_index`・サイトマップ・`_redirects` 等。 |
| 2026-03-29（追補） | `docs/assets/og-image.png` と `scripts/generate_og_png.py`、OG/Twitter/JSON-LD を PNG へ、`aria-expanded` 同期強化、フッター `.footer-ai-note`。 |
| 2026-03-29（本ドキュメント） | 上記を本ファイルに反映。「未対応」の見分け方として **表 A/B/C**・任意項目の明示・対応履歴を追記。 |
